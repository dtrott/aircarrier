...
public class Propeller extends Node implements Updatable {
	...
	public Propeller(Vector3f lightVector) {
		...
		BlendState.setSourceFunction( BlendState.SB_SRC_ALPHA );
		BlendState.setDestinationFunction( BlendState.DB_ONE_MINUS_SRC_ALPHA );
		...
		shineBlendState = display.getRenderer().createBlendState();
		shineBlendState.setBlendEnabled( true );
		shineBlendState.setSourceFunction(BlendState.SourceFunction.SourceAlpha);
		shineBlendState.setDestinationFunction(BlendState.DestinationFunction.One);
		shineBlendState.setTestEnabled(true);
		shineBlendState.setEnabled( true );
        ...
        for (int i = 0; i < propQuads.length; i++) {
			...
			prop.setRenderState(blendState);
		...
		propQuads[PART_COUNT-1].setRenderState(shineBlendState);
