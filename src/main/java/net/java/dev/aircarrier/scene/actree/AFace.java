...
public class AFace extends TriMesh {
	...
	static {
		for (int i = 0; i < 51; i++) {
			...
			occlusionTextures[i].setWrap(Texture.WM_CLAMP_S_CLAMP_T);
		}
	}
	...
	public void initialize(int face) {
        ...
        tbuf = BufferUtils.createVector2Buffer(batch.getVertexCount());
        setTextureBuffer(0, tbuf, 0);

        tbuf2 = BufferUtils.createVector2Buffer(batch.getVertexCount());
        setTextureBuffer(0, tbuf2, 1);
	    ...
	    for (int i = 0; i < batch.getTriangleCount() * 3; i++) {
			batch.getIndexBuffer().put(i);
	    }
	    
	    cbuf = BufferUtils.createFloatBuffer(4 * batch.getVertexCount());
	    setColorBuffer(0, cbuf);
	...
	private void buildTri(int face, int i) {
		...
		for (int j = 0; j < 3; j++) {
			...
			batch.getVertexBuffer().put(faceVert.x).put(faceVert.y).put(faceVert.z);
