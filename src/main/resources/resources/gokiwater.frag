...
//	vec4 fresnelTerm = 1.0 - invertedFresnel;
	...
	vec4 viewN = normalize(cameraPos - position);
	float iFresnel = viewN.z;
	float fresnel = 1.0 - fresnel;
...
//	vec4 specular = vec4(pow(intensity, kShine));
	...
	gl_FragColor = 0.5*(refractionColor + reflectionColor);// + specular; 
	
	//gl_FragColor = fresnel * reflectionColor;// + iFresnel * refractionColor;// + specular; 
	//gl_FragColor = vec4(iFresnel, iFresnel, iFresnel, 1.0);
	
	//gl_FragColor = (cameraPos-position);
